    ...
    <div ref="scrollCon" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll" class="route-box">
        ...
        <div ref="scrollBody" :style="{left: tagBodyLeft + 'px'}" class="route-inner-scroll">
            <transition-group tag="div" name="taglist-moving-animation">
                <Tag ref="tagsPageOpened" @click.native="toPage(page)" @on-close="closeHandler(page)" v-for="(page,index) in pageList" :key="page.id" :name="page.name" type="dot" class="no-select" :closable="page.name!=root.name" :color="currPageName==page.name?'blue':'default'">
                    {{page.title}}
                </Tag>
            </transition-group>
        </div>
